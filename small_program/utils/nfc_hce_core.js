...
class NfcHCECore{ 
    ...
    startNfcHCE(){ 
        var that = this 
        wx.startHCE({ aid_list: this.aids, success:function(res){ 
    ...
    sendNfcHCEMessage(hexApdu){ 
        ...
        for (var i = 0; i < dataView.byteLength; i++) { 
            ...
            wx.sendHCEMessage({ data: retbuffer, success:function(res){ console.log('NfcHCECore-->sendNfcHCEMessage::success:', res) 
            that._runCallBack(res.errCode, res.errMsg) }, 
            fail:function(err){ 
    ...
    simple(){ 
        ...
        wx.getHCEState({ 
            success:function(res){ 
                console.log('NfcHCECore-->simple::getHCEState:', res);console.log(that.aids); 
                that._runCallBack(res.errCode, res.errMsg) 
                wx.startHCE({ 
                    aid_list: that.aids, 
                    success:function(res){ 
                    ...
                    wx.onHCEMessage(function(res){ 
                        ...
                        that.nfcMessageCallBack(res.messageType, res.reason, 
                    comm.ab2hex(res.data)) 
                    }) 
                    }, 
                fail:function(err){ 
                    that.callError(err) 
